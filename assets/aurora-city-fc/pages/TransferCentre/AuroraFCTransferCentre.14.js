...
(()=>{
  ...
  function plan(){return {...defaults,...read(PLAN_KEY,{}),paydayMonthly:0};}
  ...
  function projections(p,m){
    ...
    for(let month=1;month<=Math.max(120,p.months);month++){
      ...
      annual+=(release.total*m.yieldRate);
  ...
  function missionMarkup(){return `<section id="auroraM4Mission" aria-label="Aurora M4 integrated mission control">
    <header class="m4-mission-head"><div><span class="m4-mission-kicker">Transfer Centre • master investment plan</span><h3>10-Month Investment Plan</h3><p>This is the only income simulator on the page: £3,500 in months 1–8, then £2,500 in months 9–10, for £33,000 invested in total.</p></div><span class="m4-system-chip" id="m4SystemChip"><i></i><span>Checking systems</span></span></header>
    <div class="m4-mission-grid">
      <article class="m4-stat featured"><small>Total 10-month plan</small><strong id="m4BaseCapital">—</strong><span>£25,000 existing pot + £8,000 ETF pot</span></article>
      <article class="m4-stat"><small>This month’s investment</small><strong id="m4FullCapital">—</strong><span id="m4CurrentBudgetNote">Loading the current plan month</span></article>
      <article class="m4-stat"><small>Projected final income</small><strong id="m4BaseFinal">—</strong><span id="m4BaseFinalNote">At the current portfolio yield</span></article>
      <article class="m4-stat"><small>Projected monthly income</small><strong id="m4FullFinal">—</strong><span id="m4FullFinalNote">Final annual income divided by 12</span></article>
      <article class="m4-stat"><small>Progress to £625/month</small><strong id="m4TargetProgress">—</strong><span id="m4TargetNote">Calculating plan progress</span><div class="m4-target-track"><i id="m4TargetFill"></i></div></article>
    </div>
    ...
    <details class="m4-mission-details"><summary>Edit the plan and view the 10-month schedule</summary><div class="m4-details-body">
      <div class="m4-plan-editor"><div class="m4-editor-grid">
        ...
        <input id="m4PaydayMonthly" type="hidden" value="0">
        <label class="m4-field"><span>Current plan month</span><b>#</b><input id="m4CurrentMonth" type="number" min="1" max="120" step="1"></label>
      </div><button class="m4-save" id="m4SavePlan" type="button">Save and sync the Aurora plan</button><span class="m4-editor-note">The agreed plan invests £33,000 only. The previous £48,000 comparison has been removed to avoid double-counting regular payday money.</span></div>
      <div class="m4-health-box"><div class="m4-health-list" id="m4HealthList"></div></div>
      <div class="m4-month-table-wrap"><table class="m4-month-table"><thead><tr><th>Month</th><th>Existing pot</th><th>ETF release</th><th>Total invested</th><th>Projected annual income</th><th>Projected monthly income</th></tr></thead><tbody id="m4MonthRows"></tbody></table></div>
  ...
  function renderLifecycle(life){
    ...
    const stageText=['Plan configured','Funds and route ready','Purchase execution complete','Registration confirmed','Signing live in the squad'][Math.max(0,life.stage-1)];q('m4LifecycleStatus').textContent=stageText;
  }
  ...
  function update(){
    ...
    const currentRelease=releaseAt(p,Math.min(p.months,Math.max(1,p.currentMonth)));
    q('m4BaseCapital').textContent=money(proj.baseCapital);q('m4FullCapital').textContent=money(currentRelease.total);q('m4BaseFinal').textContent=`${money(proj.baseFinal)}/yr`;q('m4FullFinal').textContent=`${money(proj.baseFinal/12)}/month`;q('m4BaseFinalNote').textContent=`Adds ${money(proj.baseFinal-m.annual)}/yr at ${(m.yieldRate*100).toFixed(2)}%`;q('m4FullFinalNote').textContent='Projected final annual income divided by 12';
    const budgetNote=q('m4CurrentBudgetNote');if(budgetNote)budgetNote.textContent=`Month ${p.currentMonth}: ${money(currentRelease.core)} existing pot + ${money(currentRelease.etf)} ETF release`;
    const finalMonthly=proj.baseFinal/12;const progress=Math.max(0,Math.min(100,finalMonthly/p.monthlyTarget*100));q('m4TargetProgress').textContent=`${progress.toFixed(1)}%`;q('m4TargetFill').style.width=`${progress}%`;q('m4TargetNote').textContent=proj.targetMonth?`The £625 target is reached during plan month ${proj.targetMonth}`:`The 10-month plan finishes at ${money(finalMonthly)}/month`;
    q('m4MonthRows').innerHTML=proj.rows.map(row=>`<tr><td><strong>Month ${row.month}</strong></td><td>${money(row.release.core)}</td><td>${money(row.release.etf)}</td><td class="m4-base">${money(row.base)}</td><td>${money(row.baseIncome)}/yr</td><td>${money(row.baseIncome/12)}</td></tr>`).join('');
    renderLifecycle(life);const chip=q('m4SystemChip');chip.className=`m4-system-chip ${h.level==='ok'?'':h.level}`.trim();chip.querySelector('span').textContent=h.label;q('m4HealthList').innerHTML=h.checks.map(x=>`<div class="m4-health-row ${x.level}"><i></i><strong>${x.name}</strong><span>${x.detail}</span></div>`).join('');q('m4Updated').textContent=`Transfer plan saved ${p.updatedAt?new Date(p.updatedAt).toLocaleString('en-GB'):'for Aurora M4'} • Live planning yield ${(m.yieldRate*100).toFixed(2)}%`;
